...
# Version 2.3: Increment count to keep in concert with other programs in the package

# Version 2.4: Increment count to keep in concert with other programs in the package

# Version 2.5: Increment count to keep in concert with other programs in the package
...
if( $paired ){
	$second_line = <SECOND>; print $second_line;
...
if ( $directory ){
	...
	my $file_name = $file_ending_elements[$item];
	$file_name=~s/_1\.fq//;
	$file_name=~s/\.fq//;
	$file_name .= ".single.fq.gz";
	$single_file = File::Spec->catpath( undef, $directory, $file_name );
}else{
	$single_file = $files[0];
	$single_file=~s/_1\.fq//;
	$single_file=~s/\.fq//;
	$single_file .= ".single.fq.gz";
}
if( -e $single_file ){
	print "error: file $single_file already exists\n";
}
open( SINGLE, "|gzip -c - >$single_file" )
...
if ( $directory ){
	...
	$file_name=~s/_1\.fq//;
	$file_name=~s/\.fq//;
	$file_name .= ".discard.fq.gz";
	$discard_file = File::Spec->catpath( undef, $directory, $file_name );
}else{
	$discard_file = $files[0] ;
	$discard_file=~s/_1\.fq//;
	$discard_file=~s/\.fq//;
	$discard_file .= ".discard.fq.gz";
}
if( -e $discard_file ){
	print "error: file $discard_file already exists\n";
}
open( DISCARD, "|gzip -c - >$discard_file" )
...
if( $paired ){
	...
	if ( $directory ){
		...
		my @file_ending_elements = split(/\//, $files[0]);
		my $item = scalar @file_ending_elements - 1;
		my $file_name = $file_ending_elements[$item] . ".paired1.fq";
		$file_name=~s/_1\.fq//;
		$paired_file1 = File::Spec->catpath( undef, $directory, $file_name );
	}else{
		$paired_file1 = $files[0] . ".paired1.fq";
		$paired_file1=~s/_1\.fq//;
	}
	$paired_file1.=".gz";
	if( -e $paired_file1 ){
		print "error: file $paired_file1 already exists\n";
	}
	open( PAIRED1, "|gzip -c - >$paired_file1" )
	...
	if ( $directory ){
		...
		my @file_ending_elements = split(/\//, $files[0]);
		my $item = scalar @file_ending_elements - 1;
		my $file_name = $file_ending_elements[$item] . ".paired2.fq";
		$file_name=~s/_1\.fq//;
		$paired_file2 = File::Spec->catpath( undef, $directory, $file_name );
	}else{
		$paired_file2 = $files[0] . ".paired2.fq";
		$paired_file2=~s/_1\.fq//;
	}
	$paired_file2.=".gz";
	if( -e $paired_file2 ){
		print "error: file $paired_file2 already exists\n";
	}
	open( PAIRED2, "|gzip -c - > $paired_file2" )
...
my $summaryname= join '.', @name[0..$#name-3];
